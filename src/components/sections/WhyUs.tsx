    ...
    <Section sectionId="about" gradient>
      ...
      <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
        {points.map(({ key, icon: Icon }, index) => (
          <div key={key} className="flex gap-4" style={{ animationDelay: `${index * 100}ms` }}>
            ...
            <div>
              <h3 className="text-heading-sm text-foreground mb-1">{t(`points.${key}.title`)}</h3>
              <p className="text-body text-muted-foreground">{t(`points.${key}.description`)}</p>
            </div>
          </div>
        ))}
      </div>
